...
ssh-keyscan kubernetes01 | sudo tee -a /root/.ssh/known_hosts
...
#sudo yum -y install --enablerepo=virt7-docker-common-release kubernetes docker etcd # etcd only on master!
# put config files!
...
#sudo kubeadm init --pod-network-cidr 10.244.0.0/16 --apiserver-advertise-address 192.168.100.101 
...
#https://192.168.100.101:6443
#kubeadm token list | grep init | awk ' { print $1 } '
#kubeadm join --token c65cab.496ce7c7ef5d26b9 192.168.100.101:6443
