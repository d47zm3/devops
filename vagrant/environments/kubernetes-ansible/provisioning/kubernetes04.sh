#!/bin/bash

# install some basics
sudo yum -y install epel-release
sudo yum -y install ansible 
sudo yum -y install git
sudo yum -y install net-tools
sudo yum -y install tmux
sudo yum -y install wget
sudo yum -y install vim
# install plugin for yaml!
sudo rm -rf /root/.ssh/
sudo mkdir /root/.ssh
sudo chmod 700 /root/.ssh
echo '-----BEGIN RSA PRIVATE KEY-----

9NngJE4Gliu3UBBJcQW8k+lbi4rALDVP3HXIyp309j7h2h9HJQeW
-----END RSA PRIVATE KEY-----' | sudo tee /root/.ssh/id_rsa

echo 'ssh-rsa  root@devops01.devops.pl' | sudo tee /root/.ssh/id_rsa.pub

echo 'ssh-rsa  root@devops01.devops.pl' | sudo tee /root/.ssh/authorized_keys

echo 'ssh-rsa  d47zm3@asus' | sudo tee -a /root/.ssh/authorized_keys


sudo chmod 600 /root/.ssh/id_rsa
sudo chmod 644 /root/.ssh/id_rsa.pub
sudo chmod 600 /root/.ssh/authorized_keys
sudo sed -i 's/^127.*/127.0.0.1 localhost localhost.localdomain localhost4 localhost4.localdomain4/g' /etc/hosts
echo "192.168.100.101 kuber-ans-01" | sudo tee -a /etc/hosts
echo "192.168.100.102 kuber-ans-02" | sudo tee -a /etc/hosts
echo "192.168.100.103 kuber-ans-03" | sudo tee -a /etc/hosts
echo "192.168.100.104 kuber-ans-04" | sudo tee -a /etc/hosts
ssh-keyscan kuber-ans-01 | sudo tee -a /root/.ssh/known_hosts
ssh-keyscan kuber-ans-02 | sudo tee -a /root/.ssh/known_hosts
ssh-keyscan kuber-ans-03 | sudo tee -a /root/.ssh/known_hosts
ssh-keyscan kuber-ans-04 | sudo tee -a /root/.ssh/known_hosts

ln -s -f /usr/share/zoneinfo/Europe/Warsaw /etc/localtime

### Tools & Roles

sudo wget https://raw.githubusercontent.com/d47zm3/vagrant/master/files/.tmux.conf -O /root/.tmux.conf

sudo yum install -y ntp
sudo systemctl enable ntpd
sudo systemctl start ntpd
sudo yum -y update
sudo systemctl stop firewalld
sudo systemctl disable firewalld
